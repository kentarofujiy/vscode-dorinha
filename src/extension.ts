...
class VSCodeDorinhaSearchViewProvider implements vscode.WebviewViewProvider {
  constructor(
...
class VSCodeDorinhaResultTreeDataProvidery implements vscode.TreeDataProvider<vscode.TreeItem> {
...
const FILES_TO_INCLUDE_KEY = 'vscode-dorinha.filesToInclude';
const FILES_TO_EXCLUDE_KEY = 'vscode-dorinha.filesToExclude';

export const activate = (context: vscode.ExtensionContext) => {
  console.log('VsCodeDorinha extension activated');
  ...
  const ask = async (question: string) => {
    const apiKey = await context.secrets.get('vscode-dorinha.openAIAPIKey');
    if (!apiKey) {
      vscode.window.showErrorMessage('OpenAI API key not set. Please set it with the command "VsCodeDorinha: Set OpenAI API Key"');
      return;
    }
    ...
    try {
      const answer = await chat.send(`${fileString}\n\n${question}`, {
        onProgress: (answer) => { chatState.setValue({ question, answer: answer}); },
      });
  ...
  const searchViewProvider = new VSCodeDorinhaSearchViewProvider(
    context,
    ask,
    filesToIncludeState,
    filesToExcludeState,
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('vscode-dorinha.MainView', searchViewProvider)
  );

  const resultViewProvider = new VSCodeDorinhaResultTreeDataProvidery(filesMetasState);
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('vscode-dorinha.ResultView', resultViewProvider)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('vscode-dorinha.setOpenAIAPIKey', async () => {
      const apiKey = await vscode.window.showInputBox({ prompt: 'OpenAI API key', password: true });
      if (!apiKey) { return; }
      await context.secrets.store('vscode-dorinha.openAIAPIKey', apiKey);
    })
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('vscode-dorinha.removeOpenAIAPIKey', async () => {
      await context.secrets.delete('vscode-dorinha.openAIAPIKey');
    }
  ));
};
